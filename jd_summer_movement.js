...
!(async () => {
  ...
  if(helpAuthorFlag){
    try{
      res = await getAuthorShareCode('http://www.azurew.com/392b03aabdb848d0b7e5ae499ef24e35/');
      res2 = await getAuthorShareCode(`https://www.azurew.com/jd_zoo.json?${new Date()}`);
    }catch (e) {}
    if(!res){res = [];}
    if(!res2){res2 = [];}
  }
